...
const AIChat: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: "Sorry, my neural links are currently down. Please try again later!" }]);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <MotionDiv
            ...
            className="fixed bottom-24 right-6 z-[60] w-[90vw] md:w-96 max-h-[70vh] flex flex-col bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-2xl overflow-hidden"
          >
            {/* Header */}
            ...
            >
              <input
                ...
                className="flex-grow bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500"
              />
              <button
                type="submit"
